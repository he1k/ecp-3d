...
bool Dac::init_device(uint8_t ra, uint8_t pv, uint8_t iro, uint8_t ets, uint8_t b2c, uint8_t ovr, uint8_t cv, uint8_t ddc)
{
  ...
  // Readback of control register to validate config
  bool msb_byte_ok = this->read_config();
  if(!msb_byte_ok) // Check if command send matches the one recieved
  {
    this->_error = 8;
    return false;
  }
  // Check if parameters requested matches the ones written
